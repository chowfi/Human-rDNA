...
full_set_path = '/Users/fionachow/Documents/NYU/CDS/Summer 2024/Human rDNA Research/Project/Human-rDNA/outputs/src_outputs/new_vcf/full_set'
...
for file_path in glob.glob(os.path.join(full_set_path, 'new_vcf_ERR*.csv')):

    file_name = os.path.basename(file_path)
    run_key = file_name.split('_')[2].split('.')[0]
    ...
    if run_key in copy_number_dict:
        ...
        df['normalized AF'] = df['AF_new'] * estimated_copy_number
...
if skipped_files:
    ...
    skipped_df.to_csv(os.path.join(full_set_path, 'skipped_files.csv'), index=False)
